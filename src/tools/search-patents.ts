...
type SearchPatentsArgs = {
  ...
  count_only: boolean;
  detail_level: "full" | "compact" | "summary";
  ...
  try {
    ...
    if (!auto_paginate || range_start > 1) {
      ...
      if (parsed.results.length < requestedWindow && parsed.results.length < parsed.totalCount) {
        steeringParts.push(
          `Returned ${parsed.results.length} results for window ${range_start}–${range_end} ` +
      ...
      if (parsed.totalCount > range_end) {
        ...
        steeringParts.push(more);
      }

      if (detail_level === "full") {
        const missingAbstracts = parsed.results.filter((r) => !r.abstract).length;
        if (missingAbstracts > 0) {
          steeringParts.push(`${missingAbstracts} result(s) have no abstract in the search index. Call get_patent_details for those documents to retrieve their abstract.`);
        }
      }
      ...
      if (steeringParts.length > 0) {
        response.steeringNote = steeringParts.join(" ");
      }
      return response;
    }
    ...
    let resultCap = max_results;
    let fullDetailCapNote: string | undefined;
    if (detail_level === "full" && resultCap > 50) {
    ...
    if (grandTotal <= 2000) {
      ...
      const { error } = await paginateInto(cql, allResults, seen, resultCap);
      if (error) paginationErrors.push(error);
      steeringNote = `Auto-paginated: fetched ${allResults.length} unique results (totalCount=${grandTotal} includes family/jurisdiction variants).`;
      if (error) {
        steeringNote += ` WARNING: ${error}. Returning partial results.`;
      } else if (allResults.length < grandTotal && allResults.length >= resultCap) {
        steeringNote += ` Stopped at max_results=${resultCap}; increase max_results to retrieve more.`;
      } else if (allResults.length < grandTotal) {
        steeringNote += ` The difference is due to EPO OPS deduplication — totalCount counts all family variants (WO + EP + US + JP for the same invention) while results are partially collapsed.`;
      }
    } else {
      ...
      let yearError = false;
      for (let year = startYear; year <= endYear && allResults.length < resultCap; year++) {
        const yearCql = buildDateCql(query, `${year}0101`, `${year}1231`);
        const beforeCount = allResults.length;
        const { pageTotal: yearTotal, error } = await paginateInto(yearCql, allResults, seen, allResults.length + (resultCap - allResults.length));
      ...
      if (yearError) {
        steeringNote += ` WARNING: Pagination stopped early due to API errors (rate limiting or timeout). Returning partial results — retry later for the full set.`;
      } else if (allResults.length < grandTotal && allResults.length >= resultCap) {
        steeringNote += ` Stopped at max_results=${resultCap}; increase max_results to retrieve more.`;
      } else if (allResults.length < grandTotal) {
        steeringNote += ` The gap between totalCount and fetched is due to EPO OPS deduplication of family variants.`;
      }
    }
    ...
    if (detail_level === "full") {
      ...
      if (missingAbstracts > 0) {
        steeringNote += ` ${missingAbstracts} result(s) have no abstract; call get_patent_details for those.`;
      }
    }
    ...
    response.steeringNote = steeringNote;
...
async function searchPatentsMcp(client: EpoClient, args: SearchPatentsArgs) {
  const { errorResult, appendThrottleInfo } = createHelpers(client);
  try {
    const enriched = appendThrottleInfo(await searchPatents(client, args));
    return {
      content: [
...
Example queries:
  ...
  ct="EP3750919" (find all patents that cite EP3750919)`,
    ...
    async (args) => searchPatentsMcp(client, args),
  );
}
